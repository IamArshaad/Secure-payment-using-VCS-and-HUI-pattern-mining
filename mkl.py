...
def PE_decrypt(secret_share1, secret_share2):
    ...
    for i in range(row):
        for j in range(column):
            cnt = overlap_matrix[2 * i][2 * j] + overlap_matrix[2 * i + 1][2 * j] + overlap_matrix[2 * i][2 * j + 1] + \
...
def convertGrayToBinary(image):
    ...
    print("likhil",grayScaleImage.shape)
...
def BLD_decrypt(secret_share1, secret_share2):
    ...
    for index in range(8):
        binaryOverlapMatrix[:, :, index], binaryExtractionMatrix[:, :, index] = PE_decrypt(
            binaryShareImage_1[:, :, index], binaryShareImage_2[:, :, index])
...
def likencrypt(p,filename,s1,s2):
    try:
        input_image = Image.open(p+filename).convert('L')
...
def likdecrypt(p,p1):
    ...
    print(type(asarray(s1)))
    print(type(asarray(s2)))

    s1=asarray(s1)
    s2=asarray(s2)

    from datetime import  datetime
    sk=datetime.now().strftime("%Y%m%d%H%M%S")


    overlap_matrix, extraction_matrix = BLD_decrypt(s1, s2)
    ...
    #
    extraction_output.save("C:\\Users\\arsha\\PycharmProjects\\Sqrp\\static\\dec.png", mode='1')
